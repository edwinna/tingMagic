 ...
 */
 (function(b) {
		...
		touchType,  //考虑到设备兼容性
		slideBox = container.find('.cp-page-slide'),
		len = slideBox.length,
    ...
    function init(){
    	slideBox.find('.cp-build').addClass('active');
    	loadEvents();
	...
	function loadEvents(){
	    ...
	    container.on('touchstart MSPointerDown pointerdown', function(e) {
		   ...
		   startMove(e);
	    }).on('touchend MSPointerUp pointerup McpestureEnd MSPointerCancel pointercancel', function(e) {
		   ops.y = 0;
		   trsEnd();
	    });
	}
    ...
    function startMove(e){
		...
		if(current == len - 1) {
			...
			slideBox.find('.cp-build').removeClass('active');
